...
def mhc_pre_gemm_sqrsum_kernel(
    ...
    for h_start in range(0, hc_hidden_size, BLOCK_H):
        ...
        x_vals = tl.load(
            x_ptr + pid_n * x_stride_n + h_offsets * x_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        ...
        fn0 = tl.load(
            fn_ptr + 0 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a0 += tl.sum(x_vals * fn0)
        fn1 = tl.load(
            fn_ptr + 1 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a1 += tl.sum(x_vals * fn1)
        fn2 = tl.load(
            fn_ptr + 2 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a2 += tl.sum(x_vals * fn2)
        fn3 = tl.load(
            fn_ptr + 3 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a3 += tl.sum(x_vals * fn3)
        fn4 = tl.load(
            fn_ptr + 4 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a4 += tl.sum(x_vals * fn4)
        fn5 = tl.load(
            fn_ptr + 5 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a5 += tl.sum(x_vals * fn5)
        fn6 = tl.load(
            fn_ptr + 6 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a6 += tl.sum(x_vals * fn6)
        fn7 = tl.load(
            fn_ptr + 7 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a7 += tl.sum(x_vals * fn7)
        fn8 = tl.load(
            fn_ptr + 8 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a8 += tl.sum(x_vals * fn8)
        fn9 = tl.load(
            fn_ptr + 9 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a9 += tl.sum(x_vals * fn9)
        fn10 = tl.load(
            fn_ptr + 10 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a10 += tl.sum(x_vals * fn10)
        fn11 = tl.load(
            fn_ptr + 11 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a11 += tl.sum(x_vals * fn11)
        fn12 = tl.load(
            fn_ptr + 12 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a12 += tl.sum(x_vals * fn12)
        fn13 = tl.load(
            fn_ptr + 13 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a13 += tl.sum(x_vals * fn13)
        fn14 = tl.load(
            fn_ptr + 14 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a14 += tl.sum(x_vals * fn14)
        fn15 = tl.load(
            fn_ptr + 15 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a15 += tl.sum(x_vals * fn15)
        fn16 = tl.load(
            fn_ptr + 16 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a16 += tl.sum(x_vals * fn16)
        fn17 = tl.load(
            fn_ptr + 17 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a17 += tl.sum(x_vals * fn17)
        fn18 = tl.load(
            fn_ptr + 18 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a18 += tl.sum(x_vals * fn18)
        fn19 = tl.load(
            fn_ptr + 19 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a19 += tl.sum(x_vals * fn19)
        fn20 = tl.load(
            fn_ptr + 20 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a20 += tl.sum(x_vals * fn20)
        fn21 = tl.load(
            fn_ptr + 21 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a21 += tl.sum(x_vals * fn21)
        fn22 = tl.load(
            fn_ptr + 22 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a22 += tl.sum(x_vals * fn22)
        fn23 = tl.load(
            fn_ptr + 23 * fn_stride_m + h_offsets * fn_stride_h, mask=h_mask, other=0.0
        ).to(tl.float32)
        a23 += tl.sum(x_vals * fn23)

    # Store results
    tl.store(sqrsum_ptr + pid_n, sq)
    base = pid_n * out_stride_n
    tl.store(out_ptr + base + 0 * out_stride_m, a0)
    tl.store(out_ptr + base + 1 * out_stride_m, a1)
    tl.store(out_ptr + base + 2 * out_stride_m, a2)
    tl.store(out_ptr + base + 3 * out_stride_m, a3)
    tl.store(out_ptr + base + 4 * out_stride_m, a4)
    tl.store(out_ptr + base + 5 * out_stride_m, a5)
    tl.store(out_ptr + base + 6 * out_stride_m, a6)
    tl.store(out_ptr + base + 7 * out_stride_m, a7)
    tl.store(out_ptr + base + 8 * out_stride_m, a8)
    tl.store(out_ptr + base + 9 * out_stride_m, a9)
    tl.store(out_ptr + base + 10 * out_stride_m, a10)
    tl.store(out_ptr + base + 11 * out_stride_m, a11)
    tl.store(out_ptr + base + 12 * out_stride_m, a12)
    tl.store(out_ptr + base + 13 * out_stride_m, a13)
    tl.store(out_ptr + base + 14 * out_stride_m, a14)
    tl.store(out_ptr + base + 15 * out_stride_m, a15)
    tl.store(out_ptr + base + 16 * out_stride_m, a16)
    tl.store(out_ptr + base + 17 * out_stride_m, a17)
    tl.store(out_ptr + base + 18 * out_stride_m, a18)
    tl.store(out_ptr + base + 19 * out_stride_m, a19)
    tl.store(out_ptr + base + 20 * out_stride_m, a20)
    tl.store(out_ptr + base + 21 * out_stride_m, a21)
    tl.store(out_ptr + base + 22 * out_stride_m, a22)
    tl.store(out_ptr + base + 23 * out_stride_m, a23)
...
def mhc_pre_fuse_kernel(
    ...
    pre_mix_0 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + 0) * rms_inv * scale_0
    ...
    pre_mix_1 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + 1) * rms_inv * scale_0
    ...
    pre_mix_2 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + 2) * rms_inv * scale_0
    ...
    pre_mix_3 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + 3) * rms_inv * scale_0
    ...
    post_0 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + HC + 0) * rms_inv * scale_1
            + tl.load(hc_base_ptr + HC + 0)
        )
        * hc_post_mult_value
    )
    post_1 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + HC + 1) * rms_inv * scale_1
            + tl.load(hc_base_ptr + HC + 1)
        )
        * hc_post_mult_value
    )
    post_2 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + HC + 2) * rms_inv * scale_1
            + tl.load(hc_base_ptr + HC + 2)
        )
        * hc_post_mult_value
    )
    post_3 = (
        tl.sigmoid(
            tl.load(gemm_out_ptr + pid_n * HC_MULT3 + HC + 3) * rms_inv * scale_1
            + tl.load(hc_base_ptr + HC + 3)
        )
        * hc_post_mult_value
    )
    ...
    comb_base = 2 * HC
    cm_00 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 0
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 0)
    cm_01 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 1
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 1)
    cm_02 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 2
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 2)
    cm_03 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 3
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 3)
    cm_10 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 4
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 4)
    cm_11 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 5
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 5)
    cm_12 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 6
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 6)
    cm_13 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 7
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 7)
    cm_20 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 8
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 8)
    cm_21 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 9
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 9)
    cm_22 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 10
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 10)
    cm_23 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 11
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 11)
    cm_30 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 12
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 12)
    cm_31 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 13
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 13)
    cm_32 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 14
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 14)
    cm_33 = tl.load(
        gemm_out_ptr + pid_n * HC_MULT3 + comb_base + 15
    ) * rms_inv * scale_2 + tl.load(hc_base_ptr + comb_base + 15)
    ...
    tl.store(comb_mix_ptr + comb_base_out + 0, cm_00)
    tl.store(comb_mix_ptr + comb_base_out + 1, cm_01)
    tl.store(comb_mix_ptr + comb_base_out + 2, cm_02)
    tl.store(comb_mix_ptr + comb_base_out + 3, cm_03)
    tl.store(comb_mix_ptr + comb_base_out + 4, cm_10)
    tl.store(comb_mix_ptr + comb_base_out + 5, cm_11)
    tl.store(comb_mix_ptr + comb_base_out + 6, cm_12)
    tl.store(comb_mix_ptr + comb_base_out + 7, cm_13)
    tl.store(comb_mix_ptr + comb_base_out + 8, cm_20)
    tl.store(comb_mix_ptr + comb_base_out + 9, cm_21)
    tl.store(comb_mix_ptr + comb_base_out + 10, cm_22)
    tl.store(comb_mix_ptr + comb_base_out + 11, cm_23)
    tl.store(comb_mix_ptr + comb_base_out + 12, cm_30)
    tl.store(comb_mix_ptr + comb_base_out + 13, cm_31)
    tl.store(comb_mix_ptr + comb_base_out + 14, cm_32)
    tl.store(comb_mix_ptr + comb_base_out + 15, cm_33)
...
def mhc_pre(
    ...
    """
    logger.debug(
        "GEMS MHC_PRE FORWARD, residual=%s, fn=%s",
        residual.shape,
        fn.shape,
    )
