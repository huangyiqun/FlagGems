...
def fill_scalar(input, value):
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_SCALAR")
...
def fill_scalar_out(input, value, *, out=None):
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_SCALAR_OUT")
    if out is None:
...
def fill_tensor(input, value):
    ...
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_TENSOR")
    if value.ndim != 0:
        raise RuntimeError(
...
def fill_tensor_out(input, value, *, out=None):
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_TENSOR_OUT")
    if out is None:
...
def fill_tensor_(self, value):
    ...
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_TENSOR_")
    if value.ndim != 0:
        raise RuntimeError(
            f"fill only supports 0-dimension value tensor but got tensor with {value.ndim} dimensions."
        )
...
def fill_scalar_(self, value):
    logger.debug("GEMS_NVIDIA GEMS_HOPPER FILL_SCALAR_")
    if self.is_contiguous():
        n_elements = self.numel()
        grid = (triton.cdiv(n_elements, 1024),)
        with torch_device_fn.device(self.device):
